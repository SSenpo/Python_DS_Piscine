...
class Research:
    ...
    class Analytics(Calculations):
        def save_file(self, data, name_of_file: str, file_format: str) -> None:
            with open(name_of_file + '.' + file_format, 'w', encoding='utf-8') as file:
                file.writelines(data)

        def predict_random(self, prediction_count: int) -> list:
            result = []

            while prediction_count:
                rand = randint(0, 1)
